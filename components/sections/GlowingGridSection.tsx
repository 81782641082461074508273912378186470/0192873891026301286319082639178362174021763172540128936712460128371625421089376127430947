...
const GridItem = ({ area, icon, title, description }: GridItemProps) => {
  return (
    <li className={`min-h-[14rem] list-none ${area}`}>
      <div className="relative h-full border border-white/50 p-5 ">
        <GlowingEffect
          blur={1}
          borderWidth={4}
          spread={80}
          ...
          inactiveZone={0.01}
        />
        <div className="relative flex h-full flex-col justify-between gap-6 overflow-hidden bg-dark-800/30 border-0.75 p-6 shadow-[0px_0px_27px_0px_#2D2D2D] md:p-6">
          <div className="relative flex flex-1 flex-col justify-between gap-3">
            <div className="w-fit border-[1px] border-white/30 bg-dark-700 p-2 ">{icon}</div>
